...
with (oPlataforma) {
    ...
    if (player.y >= y - sprite_height and player.y <= y + sprite_height) //or (player.x >= x - sprite_width/2 and player.x <= x + sprite_width/2)) {
        if player.x >= x - sprite_width/2 and player.x <= x + sprite_width/2{
			player.vspeed = 0
		}	
}
